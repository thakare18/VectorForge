...
export default function Login() {
  ...
  const handleLogin = (e) => {
    ...
    setTimeout(() => {
      dispatch(
        loginSuccess({
          user: { name: email.split("@")[0], email, joinedAt: new Date().toISOString() },
          token: "mock-jwt-local",
        })
      );
      toast.success("Signed in (mock auth)");
      navigate("/");
      setLoading(false);
    }, 800);
  };
  ...
  const oauth = (provider) => {
    dispatch(mockOAuthLogin(provider));
    ...
    <div className="grid-bg flex min-h-screen items-center justify-center p-4">
      ...
      <div className="glass-card relative z-10 w-full max-w-md p-8 fade-in">
        ...
        <div className="mt-6 space-y-3">
          ...
          <Button variant="secondary" className="w-full" onClick={() => oauth("github")}>
            <GitHubIcon />
